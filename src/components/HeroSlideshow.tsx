...
const HeroSlideshow = () => {
  ...
  return (
    <section className="relative h-screen overflow-hidden">
      ...
      <button 
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white transition-smooth p-2"
      >
        <ChevronLeft size={32} />
      </button>
      <button 
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white transition-smooth p-2"
      >
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition-smooth ${
              index === currentSlide ? 'bg-saffron' : 'bg-white/50'
            }`}
          />
        ))}
      </div>
